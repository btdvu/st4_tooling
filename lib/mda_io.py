...
def readmda(fname, folder=os.getcwd(), dtype=np.double):
    ...
    Parameters:
        fname (str):      Filename of the MDA file to read.
        folder (str):     Folder path where the MDA file is located. Defaults to current directory.
        dtype (np.dtype): Desired NumPy dtype for output. Defaults to np.double.
    
    Returns:
        raw_data (np.ndarray): n-dimensional array read from the file.
    """
...
def writemda(fname, mat):
    ...
    Parameters:
        fname (str):    Target filename to write the MDA data.
        mat (np.ndarray): The n-dimensional NumPy array to write.
